...
class Mailer < ActionMailer::Base
  ...
  def mail(headers={}, &block)
    ...
    m = if block_given?
      # super headers, &block 
      @body = super headers, &block 
    else
      super headers do |format|
        # format.text
        # format.html unless Setting.plain_text_mail?
        @body = if format.html.blank? then format.text else format.html end
      end
    end

    # m=1
    DeliveryBoy.configure do |config|
      config.client_id = "Redmine_DES"
      config.brokers = "10.234.253.129:9092"
      # ...
    end

    DeliveryBoy.deliver_async("{\"from\":\"#{replaceQuotes headers['From']}\", \"to\":\"#{replaceQuotes headers[:to]}\", \"cc\":\"#{replaceQuotes headers[:cc]}\", \"bcc\":\"#{replaceQuotes headers[:bcc]}\", \"subject\":\"#{replaceQuotes headers[:subject]}\", \"body\":\"#{replaceQuotes @body}\"}", topic:"test")
